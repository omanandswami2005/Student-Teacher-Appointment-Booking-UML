...
import { getAllStudents, deleteStudent, approveStudent, } from '../../api/adminApi';
...
await requestHandler(
  ...
  const filteredStudents = students.filter(student => student.name.toLowerCase().includes(filter.toLowerCase())||student.email.toLowerCase().includes(filter.toLowerCase()));
  ...
  return (
    ...
    <h3 className="text-xl font-semibold mb-4 dark:text-gray-200">Student List</h3>
    <input
      type="text"
    ...
    <table className="min-w-full bg-white dark:bg-gray-800">
      ...
      <tbody>
        {filteredStudents.map((student, index) => (
          <tr key={student._id} ref={index === filteredStudents.length - 1 ? lastStudentElementRef : null} className="dark:bg-gray-900">
            <td className="border px-4 py-2 dark:border-gray-700 dark:text-gray-200">{student.name}</td>
            <td className="border px-4 py-2 dark:border-gray-700 dark:text-gray-200">{student.email}</td>
            <td className="border px-4 py-2 dark:border-gray-700 dark:text-gray-200">{student.approved ? 'Approved' : 'Pending...'}</td>
